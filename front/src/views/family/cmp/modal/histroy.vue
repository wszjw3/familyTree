	...
	<el-dialog :title="computedTitle" :visible.sync="isShow" width="60%">
      ...
      <el-tab-pane label="捐献流水" name="first">
        <el-table :data="histroy.data">
          ...
          <el-table-column prop="donate_sum_amt" label="总捐献金额"></el-table-column>
        </el-table>
        <el-pagination
          style="float:right"
      ...
      <el-tab-pane label="捐献排名" name="second">
        <el-table :data="rank.data">
          ...
          <el-table-column prop="user_name" label="姓名"></el-table-column>
          <el-table-column prop="donate_sumnumber" label="捐献次数"></el-table-column>
          <el-table-column prop="donate_amt" label="捐献总金额"></el-table-column>
        </el-table>
        <el-pagination
          style="float:right"
...
export default {
	...
	computed: {
		...
		computedTitle () {
			return '捐献记录     当前已有' + this.statistics.recordnumber + '人捐赠，捐赠总基金 ¥ :' + this.statistics.trans_amt
		}
	},
	watch: {
		value(val) {
    ...
    getTotal () {
			Family.familyDonateRecordStatistics({family_id: this.$router.currentRoute.query.familyId}).then(res => {
